...
FileRule.mimeTypes = new Map([
  ...
  ["application/octet-stream", ["psd", "xcf"]],
  ...
  ["image/gif", ["gif"]],
  ["image/heic", ["heic"]],
  ["image/heic-sequence", ["heic", "heics"]],
  ["image/heif", ["heic", "heif"]],
  ["image/heif-sequence", ["heic", "heifs"]],
...
FileRule.convertMimeToExt = (mime) => {
  ...
  const found = mime.match(/^(?<toplevel>[a-z]+)\/(?<sub>[*]|[a-z0-9.+-]+)$/i);
  ...
  if (found) {
    ...
    for (const [key, value] of FileRule.mimeTypes) {
      if (key === found[0] || ("*" === sub && key.startsWith(toplevel + "/"))) {
        results.push(...value);
      }
    }
  }
